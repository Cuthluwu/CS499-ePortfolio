...
public final class TaskService implements RecordRepository<Task> {
    private static final Comparator<Task> SEARCH_ORDER = Comparator
            .comparing(Task::getName, String.CASE_INSENSITIVE_ORDER)
    ...
    /**
     * Searches task names and descriptions and returns matches in predictable
     * name-and-ID order.
     */
    public List<Task> searchByKeyword(String keyword) {
        String normalized = Validation.searchTerm(keyword, "Task search").toLowerCase(Locale.ROOT);
        return tasks.values().stream()
                .filter(task -> task.getName().toLowerCase(Locale.ROOT).contains(normalized)
                        || task.getDescription().toLowerCase(Locale.ROOT).contains(normalized))
                .map(Task::new)
                .sorted(SEARCH_ORDER)
                .toList();
    }
