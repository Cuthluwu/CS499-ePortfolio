...
4. Read every legacy account. Hash the stored legacy password with a unique salt and record the
   ...
   accounts are preserved even if an old password would not satisfy the new minimum length. If
   a legacy account has no password, assign an independently randomized 256-bit inaccessible
   credential instead of deriving a guessable value from the username.
