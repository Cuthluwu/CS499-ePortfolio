...
1. `PasswordHasher`, `InputValidator`, and `LegacyDataNormalizer` compiled with Java 17 and
   `-Xlint:all`. A standalone test passed 36 checks, including upper-bound iteration handling,
   verifier dimensions, and randomized inaccessible legacy credentials.
...
3. Every Android XML resource was parsed, every `R.id` reference in the Java source was matched
   ...
   SHA-256. The prior Android/JUnit stub compilation remains recorded in
   `STATIC_PROJECT_VERIFICATION.txt`; the final pure-Java security changes were recompiled with
   lint enabled. This does not replace a real Android SDK build, and it is labeled accordingly.
...
- `FINAL_SOURCE_SYNTAX_RESULTS.txt` and `run_final_source_syntax_check.sh`
